...
def generate_boxed_calendar(year: int) -> str:
    ...
    # Arrange months in 4 per row (3 rows)
...
def generate_full_calendar(start=2000, end=2080, step=2, filename="Boxed_Calendar_2000_to_2080.txt"):
    with open(filename, "w", encoding="utf-8") as f:  # ✅ Fix encoding issue
        for year in range(start, end + 1, step):
            boxed = generate_boxed_calendar(year)
            f.write(boxed + "\n\n")
    print(f"✅ Boxed calendar saved to '{filename}'")
